...
let pianoInitializing = false; // Verrou pour éviter les initialisations multiples
let initPromise = null; // Pour partager la promesse d'initialisation

function initializePiano() {
  if (piano && pianoLoaded) return Promise.resolve();
  if (pianoInitializing && initPromise) {
    // Si déjà en cours d'initialisation, retourner la même promesse
    return initPromise;
  }
  
  pianoInitializing = true;
  ...
  initPromise = Tone.loaded().then(() => {
    pianoLoaded = true;
    pianoInitializing = false;
    console.log('✅ Piano haute qualité chargé avec reverb');
  }).catch(err => {
    pianoInitializing = false;
    console.error('Erreur chargement piano:', err);
    throw err;
  });
...
function preloadAudio() {
  ...
  const startAudio = () => {
    console.log('🎵 Première interaction détectée, initialisation audio...');
    
    // Démarrer Tone.js immédiatement
    if (Tone.context.state !== 'running') {
      Tone.start().then(() => {
        console.log('🔊 Tone.js démarré');
        // Puis initialiser le piano
        if (!piano) {
          initializePiano().then(() => {
            console.log('🎹 Piano prêt après première interaction');
          }).catch(err => {
            console.error('Erreur initialisation piano:', err);
          });
        }
      }).catch(err => {
        console.error('Erreur Tone.start:', err);
      });
    } else if (!piano) {
      // Tone.js déjà actif, juste initialiser le piano
      initializePiano().then(() => {
        console.log('🎹 Piano prêt après première interaction');
      }).catch(err => {
        console.error('Erreur initialisation piano:', err);
  ...
  // Ajouter les listeners pour la première interaction (avec options pour mobile)
...
function playNoteSound(note, duration = 1.0, startTime = 0, transpose = 0) {
  ...
  if (!piano || !pianoLoaded) {
    // Démarrer Tone.js immédiatement si nécessaire (crucial pour mobile)
    if (!piano && Tone.context.state !== 'running') {
      Tone.start().catch(err => console.log('Tone.start error:', err));
    }
    
    initializePiano().then(() => {
      // Une fois initialisé, jouer la note
      if (Tone.context.state !== 'running') {
        Tone.start().then(() => {
          const now = Tone.now();
          piano.triggerAttackRelease(finalNote, duration, now + startTime);
        }).catch(err => console.log('Erreur Tone.start:', err));
      } else {
        const now = Tone.now();
        piano.triggerAttackRelease(finalNote, duration, now + startTime);
      }
