...
// - PC inchangé pour tous (x=-20, y=y+3)
// Corrections v2.2 : 
...
function drawMusicalStaff(notes, chordNotation = '') {
  ...
  const staffY = 50; // Descendu de 35 à 50 pour plus d'espace en haut (notes hautes)
...
//   drawKeySignature(svg, armatureStartX, staffY, lineSpacing, keySignature);
  ...
  notePositions.forEach((pos, idx) => {
    ...
    const doubleFlatXOffset = isMobile ? -16 : -18; // Mobile : éloigné (-16), PC : éloigné (-18)
...
function drawTrebleClef(svg, x, y) {
  // Clé de sol responsive : différentes tailles selon l'appareil
  const clef = document.createElementNS('http://www.w3.org/2000/svg', 'text');
  clef.setAttribute('x', x);
  clef.setAttribute('data-clef-version', 'v2.8-iphone-fix');
  ...
  if (isIOS) {
    ...
    clef.setAttribute('y', yPos);
    clef.setAttribute('font-size', '42');
    console.log('TABLET MODE: y=' + yPos + ', font-size=42');
  } else {
    // PC : taille 58
    const yPos = y + 9;
    clef.setAttribute('y', yPos);
