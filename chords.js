// chords.js v2.5 - Génération de tous les accords
// Correction v2.5 : 
// - Correction du calcul d'octave pour add13 (et toutes les extensions) : 
//   détection quand la note cible "redescend" chromatiquement par rapport à la fondamentale
// Correction v2.4 : 
...
function getNoteName(rootNote, interval, quality = null) {
  ...
  // CORRECTION v2.5: Calculer l'octave en tenant compte de la position chromatique
  // Si la note cible est "avant" la fondamentale dans l'octave, il faut ajouter +1
  let octave = Math.floor(interval / 12);
  
  // Calculer les positions chromatiques (0-11)
  const rootChromaticPos = (sharpNotes.indexOf(rootNote) !== -1 ? sharpNotes.indexOf(rootNote) : flatNotes.indexOf(rootNote)) % 12;
  const targetChromaticPos = ((rootChromaticPos + interval) % 12 + 12) % 12;
  
  // Si on est dans une extension (interval >= 12) ET que la note cible est "avant" la fondamentale
  // chromatiquement, alors il faut ajouter +1 à l'octave
  if (interval >= 12 && targetChromaticPos < rootChromaticPos) {
    octave += 1;
  }
  ...
  ROOT_NOTES_NATURAL.forEach(rootNote => {
    [''].concat(ALTERATIONS).forEach(alt => {
      ...
      const allQualities = [
        ...
        'maj7#5', 'maj7b5', 'maj7#11', 'maj7#5#11', 'maj7#9',
    'mmaj7#11',
    'm9b5'
      ];
